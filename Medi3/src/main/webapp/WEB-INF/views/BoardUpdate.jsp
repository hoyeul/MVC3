...
<body>
	...
	<main>
		<form action= "BoardUpdate?c_num=<%=result.getC_num() %>" method ="post" name= "en_frm"  class="boardContent">
		       <table>
		        <caption>문의함</caption>
		        <thead class="content_head">
		            <tr class="content_head__title">
		                <td><label> 제  목  :</label></td>
		                <td><input type="text" name="title" placeholder="제목을 입력해주세요" value="<%=result.getTitle()%>"></td>
		            </tr>
		        ...
		        <tbody  class="content-text">
		            <tr >
		                <td  class="content_text__content" colspan="2">
		                    <textarea name="content" ><%=result.getContent() %></textarea>
		                </td>
		            </tr>
		            <tr>
		                <td class="content_btn" colspan="2">
		                    <button>수정</button>
		                    <button type="button"onclick="location.href='BordDelete?c_num=<%=result.getC_num() %>'">삭제</button>
		    ...
		    <input type="hidden" name="id" value="<%=result.getC_name()%>">  
		</form>
	</main>
  <footer>
                <!--
